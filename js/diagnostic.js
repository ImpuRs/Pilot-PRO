...
function _renderDiagnosticCellPanel(key,cellArts){
  ...
  if(v2.status!=='lock'&&(v2.perdus>0||(v2.prospects||0)>0)){const cliLabel=v2.perdus>0?`${v2.perdus} perdu${v2.perdus>1?'s':''} avec historique`:`${v2.prospects} prospect${v2.prospects>1?'s':''} métier`;const potLabel=v2.potentiel>0?' — potentiel '+formatEuro(v2.potentiel):'';acts.push({priority:3,src:'👥',label:`Démarcher ${cliLabel}${potLabel}`,fn:()=>{closeDiagnostic();window.scrollTo(0,0);const _mc2=document.getElementById('mainContent');if(_mc2){_mc2.style.overflow='';_mc2.scrollTop=0;}switchTab('territoire');let _lt2=-1,_tr2=0;const _pv2=setInterval(()=>{const mc=document.getElementById('mainContent');const el=document.getElementById('terrCockpitClient');if(!mc||!el){if(++_tr2>40)clearInterval(_pv2);return;}let e=el,t=0;while(e&&e!==mc){t+=e.offsetTop;e=e.offsetParent;}if((t===_lt2&&t>0)||_tr2++>40){clearInterval(_pv2);window.scrollTo(0,0);mc.scrollTo({top:t-16,behavior:'smooth'});}else _lt2=t;},100);}});}
...
function _diagGenActions(famille,v1,v2,v3){
  ...
  if(v2&&v2.status!=='lock'&&v2.perdus>0){
    ...
    acts.push({priority:3,src:'👥',label:`Démarcher ${v2.perdus} client${v2.perdus>1?'s':''} perdus${potLabel?' — potentiel '+potLabel:''}`,fn:()=>{closeDiagnostic();window.scrollTo(0,0);const _mc3=document.getElementById('mainContent');if(_mc3){_mc3.style.overflow='';_mc3.scrollTop=0;}switchTab('territoire');let _lt3=-1,_tr3=0;const _pv3=setInterval(()=>{const mc=document.getElementById('mainContent');const el=document.getElementById('terrCockpitClient');if(!mc||!el){if(++_tr3>40)clearInterval(_pv3);return;}let e=el,t=0;while(e&&e!==mc){t+=e.offsetTop;e=e.offsetParent;}if((t===_lt3&&t>0)||_tr3++>40){clearInterval(_pv3);window.scrollTo(0,0);mc.scrollTo({top:t-16,behavior:'smooth'});}else _lt3=t;},100);}});
  }
  // 🔭 LE RÉSEAU actions
  if(v3&&v3.status!=='lock'){
