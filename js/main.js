...
const fl=l=>q?l.filter(x=>(x.code+' '+x.lib).toLowerCase().includes(q)):l;const fM=fl(missed),fU=fl(under),fO=fl(over);
  ...
  function renderObsArticleSearch(){
    ...
    res.innerHTML=rows.length?`<div class="s-card border rounded-xl overflow-hidden shadow-sm mt-1">${rows.join('')}</div>`:'<p class="text-sm t-disabled mt-2 text-center py-2">Cet article n\'est pas dans la comparaison réseau — il n\'est ni manqué, ni sous-perf, ni sur-perf. Médiane réseau probablement 0.</p>';
  }
